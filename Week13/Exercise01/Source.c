...
/**
* @brief           그래프 g에 대해 다익스트라 알고리즘을 수행한 후 결과를 출력하는 함수
*
* @param g         그래프 구조체
* @param origin    출발 정점의 인덱스
*/
void Dijkstra(const Graph* g, const int origin){
	int i, j;    //
	int steps;  // 반복 횟수 

	// visitied[v] := 현재 다익스트라 탐색 과정에서 정점 v가 이미 방문 되었는지 여부 T/F
...
int main(){
	...
	// 아래 두 테스트케이스 함수를 차례로 수행하여 결과를 캡처해주세요
